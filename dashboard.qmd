---
title: "Footy Forest power ratings"
format: 
  dashboard:
...
ladder <- ladder |> 
  rename("Ladder position" = position, Team = team.name, "Games played" = played, Points = thisSeasonRecord.aggregatePoints, Percentage = thisSeasonRecord.percentage, "Ladder position this time last year" = thisRoundLastSeason.ladderPosition)
...
ladder <- ladder |> select("Ladder position", Team, Points, Percentage, "Ladder position this time last year")

ladder <- left_join(ladder, power_rankings_dashboard_for_joining, by = "Team") 

ladder <- ladder |>  rename("Power" = last_col())

ladder <- ladder |> arrange(desc(Power))
...
ladder |> 
  ...
  gt_plt_sparkline("Power trend", type = "default", same_limit = FALSE, label = FALSE, palette = c("black", "transparent","#A23333","#003300","transparent")) |> 
  opt_interactive(use_resizers = TRUE, use_highlight = TRUE, use_page_size_select = FALSE, use_pagination = FALSE) 
